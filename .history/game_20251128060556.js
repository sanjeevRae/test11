...
window.addEventListener('load', () => {
    ...
    function resetGame() {
        ...
        // place character on top of ground (character.y is sprite top)
    ...
    function spawnObstacle() {
        ...
        const idx = Math.floor(Math.random() * Math.max(1, imgs.length));
            // reduced obstacle height so player can clear more easily
            const h = Math.max(28, Math.round(displayHeight * 0.14));
            const w = Math.round(h * 0.9);
        // avoid spawning obstacles too close together
        const minGap = Math.round(displayWidth * 0.45); // min distance between obstacles (pixels)
        if (obstacles.length > 0) {
            const last = obstacles[obstacles.length - 1];
            // if last obstacle is still near the spawn edge, skip spawning now
            if (last.x > displayWidth - minGap) return;
        }
        // position obstacle so its bottom sits on the ground
        obstacles.push({ x: displayWidth + 20, y: groundY - h, width: w, height: h, imgIdx: idx });
    }

    function spawnCoin() {
        const size = Math.round(displayHeight * 0.16) || 48;
        const value = 10;
        // position coin above the ground relative to top coordinate
        coins.push({ x: displayWidth + 20, y: groundY - 80 - Math.random() * 60, width: size, height: size, value });
    }

    function isColliding(a, b) {
        // use a small padding to make collisions a bit more forgiving
            const padA = Math.max(2, Math.round(Math.min(a.width, a.height) * 0.08));
            const padB = Math.max(2, Math.round(Math.min(b.width, b.height) * 0.05));
        const ax = a.x + padA, ay = a.y + padA, aw = a.width - padA * 2, ah = a.height - padA * 2;
        const bx = b.x + padB, by = b.y + padB, bw = b.width - padB * 2, bh = b.height - padB * 2;
    ...
    if (frame % 1000 === 0) speed += 0.15;
        ...
        if (character.jumping) {
            ...
            // character.y is the sprite top; ground top is groundY, so bottom check is groundY - character.height
    ...
    // ground (groundY is top of the ground area)
    const roadY = groundY + 6;
        ctx.fillStyle = '#3a3a3a';
        ctx.fillRect(0, roadY, displayWidth, displayHeight - roadY);
        ctx.fillStyle = '#6b6b6b';
        ctx.fillRect(0, roadY - 8, displayWidth, 8);

    // character (draw at top coordinate)
    if (images.character && images.character.complete) ctx.drawImage(images.character, character.x, character.y, character.width, character.height);
    else { ctx.fillStyle = '#2e7d32'; ctx.fillRect(character.x, character.y, character.width, character.height); }
    ...
    function jump() { if (gameOver) return; if (!character.jumping) { character.jumping = true; character.vy = -22; } }
    ...
    // character.y is the sprite top, place on ground
    character.y = groundY - character.height;
