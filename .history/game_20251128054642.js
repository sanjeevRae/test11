...
window.addEventListener('load', () => {
    ...
    function endGame() {
        ...
        try {
            const prev = parseInt(localStorage.getItem('highScore') || '0', 10) || 0;
            if (score > prev) {
                localStorage.setItem('highScore', String(score));
                if (highEl) highEl.textContent = `High Score: ${score}`;
            } else {
                if (highEl) highEl.textContent = `High Score: ${prev}`;
            }
        } catch (e) {
            if (highEl) highEl.textContent = `High Score: n/a`;
        }
    }
